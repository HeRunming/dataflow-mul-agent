...
onMounted(async () => { try { backendMode.value = (await api('/api/v1/health')).backend; await Promise.all([loadResources(), loadDatasets(), refreshRuns()]); await ensureConversation(); if (selectedId.value) connectRunStream(selectedId.value) } catch (err) { error.value = err.message }; timer = setInterval(tick, 5000) })
onUnmounted(() => { clearInterval(timer); runStream?.close() })
</script>
...
<template>
  <div class="app-shell"><header class="topbar"><div class="brand"><span class="brand-mark">DF</span><div><strong>DataFlow</strong><small>Multi-Codex Studio</small></div></div><div class="top-actions"><span class="connection"><i></i>{{ backendMode }} backend</span><button class="ghost" @click="showDataset = true">Datasets ({{ datasets.length }})</button><button class="ghost" @click="showResource = true">Serving / API ({{ resources.length }})</button><button class="ghost" @click="refreshRuns">Refresh</button></div></header>
    <main class="workspace"><aside class="sidebar panel"><div class="panel-title"><span>Runs</span><span class="count">{{ runs.length }}</span></div><div class="run-list" tabindex="0" role="region" aria-label="运行记录"><button v-for="run in runs" :key="run.run_id" class="run-item" :class="{ active: run.run_id === selectedId }" @click="selectRun(run.run_id)"><span class="run-dot" :class="run.state.toLowerCase()"></span><span class="run-copy"><b>{{ run.request || 'Untitled run' }}</b><small>{{ run.run_id }} · {{ labelState(run.state) }}</small></span><span class="run-delete" :aria-disabled="deletingRuns.has(run.run_id)" :title="deletingRuns.has(run.run_id) ? 'Deleting…' : 'Delete run'" @click.stop="deleteRun(run.run_id)">{{ deletingRuns.has(run.run_id) ? '…' : '×' }}</span></button><div v-if="!runs.length" class="empty">No runs yet</div></div></aside>
      ...
      <aside class="rightbar"><div v-if="collaboration" class="panel side-panel collaboration-panel"><div class="panel-title"><span>Agent collaboration</span><span class="live"><i></i> Live</span></div><div class="agent-collab"><button v-for="agent in collaboration.agents" :key="agent.agent" class="collab-agent" :class="agent.state" @click="selectedInspector = agent; inspectorTab = 'agent'"><span class="collab-dot"></span><b>{{ agent.agent }}</b><small>{{ agent.state }} · {{ agent.events }} events</small></button></div><div v-if="selectedInspector" class="inspector"><div class="inspector-tabs"><button v-for="tab in ['agent','skill','evidence']" :key="tab" :class="{active: inspectorTab === tab}" @click="inspectorTab = tab">{{ tab }}</button></div><div v-if="inspectorTab === 'agent'" class="inspector-content"><b>{{ selectedInspector?.agent }}</b><span>State: {{ selectedInspector?.state }}</span><span>Events: {{ selectedInspector?.events }}</span><span>Jobs: {{ selectedInspector?.jobs?.join(', ') || '—' }}</span><span>Skills: {{ selectedInspector?.skills?.join(', ') || 'No skill event recorded' }}</span></div><div v-else-if="inspectorTab === 'skill'" class="inspector-list"><div v-for="skill in runSkills" :key="skill.name" class="inspector-row"><b>{{ skill.name }}</b><span>{{ skill.purpose }}</span><small>sha256: {{ skill.hash || 'unavailable' }} · calls {{ skill.calls.length }}</small></div><div v-if="!runSkills.length" class="empty">No skill metadata available.</div></div><div v-else class="inspector-list"><div v-for="artifact in runEvidence" :key="artifact.name" class="inspector-row"><b>{{ artifact.name }}</b><span>{{ artifact.available ? 'available' : 'not produced' }}</span><small v-if="artifact.value">{{ JSON.stringify(artifact.value).slice(0, 240) }}</small></div></div></div></div><div class="panel side-panel"><div class="panel-title"><span>Agent activity</span><span class="live"><i></i> Live</span></div><div class="timeline"><div v-for="event in events.slice(-14).reverse()" :key="event.seq" class="timeline-item"><span class="timeline-line"></span><span class="timeline-dot"></span><div><b>{{ eventLabel(event.event) }}</b><small>{{ event.agent }}<span v-if="event.job"> · {{ event.job }}</span></small></div></div><div v-if="!events.length" class="empty">Events will appear as agents work.</div></div></div><div v-if="agentOutputs.length" class="panel side-panel"><div class="panel-title">Codex latest output</div><div class="codex-output"><div v-for="output in agentOutputs" :key="output.agent + output.attempt"><b>{{ output.agent }}</b><pre>{{ output.text }}</pre></div></div></div></aside></main>
